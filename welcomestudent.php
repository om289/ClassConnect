    ...
    <div class="container mt-4">
        ...
        <div class="dashboard-container">
            ...
            <div class="sidebar">
                ...
                <button onclick="location.href='viewquery.php?eno=<?php echo $_SESSION['seno']; ?>'">My Query</button>
                ...
                <button onclick="location.href='viewclassnotes.php'">Class Notes & Materials</button>
                <button onclick="location.href='joinclassroom.php'" class="btn btn-primary mt-3">Join Classroom</button>
            </div>
            ...
            <div class="main-content">
                ...
                <section class="dashboard">
                    <div class="dashboard-sections">
                        ...
                        // Debugging Today's Schedule Section
                        $today = date('l'); // Gets current day name (e.g., Monday)
                        $sql = "SELECT * FROM schedules WHERE day = '$today' ORDER BY start_time";
                        $result = mysqli_query($conn, $sql);

                        if (!$result) {
                            echo '<p>Error: Unable to fetch schedule. ' . mysqli_error($conn) . '</p>';
                        ...
                        <div class="progress">
                            <h3>Last 20 Days</h3>
                            <?php
                            // Corrected query for "Last 20 Days"
                    ...
                    <section class="announcements">
                        ...
                        $sql_announcements = "SELECT ca.title, ca.content, ca.created_at, c.subject 
                        ...
                        $stmt_announcements->bind_param("ss", $yesterday, $today);
                        $stmt_announcements->execute();
                        $announcements = $stmt_announcements->get_result();

                        // Debugging: Log the date range for announcements
                        error_log("Fetching announcements between $yesterday and $today");

                        // Debugging: Log the query results
                        if ($announcements->num_rows > 0) {
                            while ($announcement = $announcements->fetch_assoc()) {
                                error_log("Announcement: " . print_r($announcement, true));
                            }
                        } else {
                            error_log("No announcements found in the last 24 hours.");
                        }
                            ...
                            <ul class="list-group">
                                <?php while ($announcement = $announcements->fetch_assoc()): ?>
                                    <li class="list-group-item">
                                        ...
                                        <small>Classroom: <?php echo htmlspecialchars($announcement['subject']); ?> | Posted on: <?php echo htmlspecialchars($announcement['created_at']); ?></small>
                                    </li>
                                <?php endwhile; ?>
                            </ul>
                        <?php else: ?>
                            <p>No announcements in the last 24 hours.</p>
                        <?php endif; ?>
                    </section>
                </section>
            </div>
